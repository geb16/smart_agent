...
class ExecutorAgent:
    ...
    def __init__(self, stm: ShortTermMemory, ltm: LongTermMemory, epi: EpisodicMemory) -> None:
        ...
        async def _run_tools_parallel(self, tool_names: List[str], base_args: Dict[str, Any]) -> List[Dict[str, Any]]:
            """
            Run multiple tools in parallel using asyncio.to_thread.
            Each tool receives filtered + preference-merged args.
            """
            ...
            async def run_single_tool(name: str) -> Dict[str, Any]:
                ...
                # filter args based on signature
                sig = inspect.signature(tool_fn)
                allowed = set(sig.parameters.keys())
                args = {k: v for k, v in args.items() if k in allowed}

                # compound interest schema fix
                ...
                # run sync tool in a worker thread
    ...
    def _run_single_step(self, step: Dict[str, Any], user_input: str) -> Any:
        ...
        if action == "tool":
            ...
            # -------------------------------------------
            # 24.1.C: Case 1: → Support multiple tools in one step
            # -------------------------------------------
            if isinstance(tool_name_field, list):
                try:
                    ...
                    for name in tool_name_field:
                        ...
                        result = self._run_single_step(sub_step, user_input)
                    
                    return {
                        "type": "tool_batch_result",
                        "tools": tool_name_field,
                        "results": results,
                    }
            # --------------------------------------------------------
            # 24.1.C: Case 2: Single tool execution(existing behavior)
            #--------------------------------------------------------
            ...
            # inject and merge global prefs
            global_prefs = self.ltm.all_prefs()
            for k, v in global_prefs.items():
                args.setdefault(k, v)

            # ---------------------------------
            # Enforcing Signature: filter args to function signature
            # ---------------------------------
            sig = inspect.signature(tool_fn)
            allowed = set(sig.parameters.keys())
            #🔖
